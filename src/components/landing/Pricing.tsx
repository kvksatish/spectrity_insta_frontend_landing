      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
          {config.plans.map((plan, index) => (
            <Card
              key={index}
              className={plan.highlighted ? "border-primary/50 border-2 shadow-md bg-background/50 backdrop-blur" : "border border-border/40 shadow-sm bg-background/50 backdrop-blur"}
            >
              <CardHeader className="pb-8 pt-8">
                <CardTitle className="text-xl font-semibold">{plan.name}</CardTitle>
                <CardDescription className="text-sm text-muted-foreground/70">{plan.description}</CardDescription>
                <div className="mt-6">
